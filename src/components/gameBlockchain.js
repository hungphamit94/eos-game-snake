...
class GameBlockchain extends Component {
    ...
    constructor(props){
        ...
        this.loadPositionPrey = this.loadPositionPrey.bind(this);
        this.loadPositionPreySpecial = this.loadPositionPreySpecial.bind(this);
    ...
    loadUser() {
        ...
        console.logog('toi muon biet:'+name);
    ...
    loadPositionPreySpecial(){
        const { setUser, user: { name } } = this.props;
        return ApiService.getUserByName(name).then(user => {
            this.setState({startPreySpecial: user.game.preySpecial.position});
            setUser({
                game: user.game,
            });
        });
    ...
    checkEatPrey(){
        if(this.state.showPreySpecial){
            if(this.state.arraySnake.indexOf(this.state.startPreySpecial)!==-1){
                ...
                // ApiService.updatescore(this.state.scorePreySpecial).then(()=>{});
...
export default connect(mapStateToProps, mapDispatchToProps)(GameBlockchain);